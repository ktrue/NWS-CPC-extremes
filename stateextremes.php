...
############################################################################
#
#   Project:    State Extremes (for one state)
#   Module:     stateextremes.php
...
#      CHMOD 666 for usaextremesCache4.txt
...
#      $statelow
#      $stateprecip 
#      $stateReportDate (nicely formatted date of the report)
#
############################################################################
// version
...
// Version 4.03b - 21-Nov-2016 - added selector for State extreme only
// Version 4.03c - 06-Feb-2019 - use https to www.cpc.noaa.gov
...
$cache_life = '3600';
$stateReportDateFormat = "l, F j, Y"; // Day, Month d, yyyy 
$tUOM = '&deg;F'; // or ='' for no temperature unit display
$rUOM = 'in';     // or ='' for no rain unit display
$ignoreStations = array( ); // Station numbers to ignore, Array of station names
...
echo "<!-- State Extremes Script Version $stateextremesverion saratoga-weather.org -->\n";
...
if(isset($_REQUEST['state'])) {
	$tstate = $_REQUEST['state'];
	$tstate = substr($tstate,0,2); // take only 2 characters
	$tstate = strtoupper($tstate); // make sure it's uppercase
	if(preg_match('|^([A-Z]{2})$|',$tstate)) {
		$selectState = $tstate;
		print "<!-- using '$selectState' for state selection -->\n";
	}
}
$selectState = ' '.trim($selectState).' '; // put state in proper format for selection
$statehighStation = '';
$statehighValue = -999;
$statelowStation = '';
$statelowValue = 999;
...
foreach ($rawrecs as $n => $rec) {
	if($stateReportDate == '' and preg_match('/^\s+(\d+)$/',$rec,$matches)) {
		$stateReportDate = gmdate("l, F j, Y",strtotime($matches[1].'T1200'));
		print "<!-- reportDate='$stateReportDate' -->\n";
		continue;
	}
	if(strlen($rec) < 30) { continue; }
	if(stripos($rec,$selectState) === false ) { continue; }
	if(check_ignore_station2($rec,$ignoreStations)) { 
	  //print "<!-- ignored $rec -->\n";
	  continue;
    }
...
foreach ($usaStations as $n => $vals) {
	...
	if($tHigh > $statehighValue) {
		if(isset($_REQUEST['debug'])) {
			print "<!-- new HIGH: '$tHigh' $n - $tState $tStation -->\n";
		}
		$statehighValue = $tHigh;
		$statehighStation = $n; // remember for later
	}
	if($tLow < $statelowValue) {
		if(isset($_REQUEST['debug'])) {
			print "<!-- new LOW: '$tLow' $n - $tState $tStation -->\n";
		}
		$statelowValue = $tLow;
		$statelowStation = $n; // remember for later
	}
	if($tPrecip > $stateprecipValue) {
		if(isset($_REQUEST['debug'])) {
			print "<!-- new Precip: '$tPrecip' $n - $tState $tStation -->\n";
		}
		$stateprecipValue = $tPrecip;
		$stateprecipStation = $n;
...
if(isset($_REQUEST['debug'])) {
  ...
  print "<!-- statehighValue=$statehighValue station=$statehighStation ".$usaStations[$statehighStation]." -->\n";
  print "<!-- statelowValue=$statelowValue station=$statelowStation ".$usaStations[$statelowStation]." -->\n";
  print "<!-- stateprecipValue=$stateprecipValue station=$stateprecipStation ".$usaStations[$stateprecipStation]." -->\n";
}
...
foreach ($usaStations as $n => $vals) {
	...
	if($tHigh == $statehighValue) {
		$statehigh .= "<b>$tHigh$tUOM at $tStation, $tState</b>\n";
	}
	if($tLow == $statelowValue) {
		$statelow .= "<b>$tLow$tUOM at $tStation, $tState</b>\n";
	}
	if($tPrecip == $stateprecipValue and $tPrecip > 0) {
		$tPrecipFmt = sprintf("%01.2f",$tPrecip/100);
		$stateprecip .= "<b>$tPrecipFmt$rUOM at $tStation, $tState</b>\n";
	}
}
// Make results HTML pretty

$statehigh   = str_replace("\n","<br/>\n",$statehigh);
$statelow    = str_replace("\n","<br/>\n",$statelow);
$stateprecip = str_replace("\n","<br/>\n",$stateprecip);
if(strlen($stateprecip) < 3) {
	$stateprecip = "No Precipitation Reported for ".trim($selectState)."<br/>\n";
}

print "<!-- statehigh  ='$statehigh' -->\n";
print "<!-- statelow   ='$statelow' -->\n";
print "<!-- stateprecip='$stateprecip' -->\n";
...
function check_ignore_station2($str, array $arr)
{
    foreach($arr as $a) {
